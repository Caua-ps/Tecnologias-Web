...
async function pass(req, res, data) {
  ...
  // Se tem keepPlaying, NUNCA pode passar - deve lançar o dado novamente
  if (gameData.dice.keepPlaying) {
    res.writeHead(400, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ error: 'You must roll the dice again (keepPlaying)' }));
    return;
  }
  
  // Verificar se existem jogadas válidas (caso normal, sem keepPlaying)
  if (canMove) {
    res.writeHead(400, { 'Content-Type': 'application/json' });
