...
export class Game {
  ...
  _move(sr, sc, tr, tc) {
    ...
    const isAI = this.current === "vermelho";


    // captura automática
    if (to && to !== 0) {
      playSound(isAI ? "capture_ai" : "capture");
  ...
  _isGameOver() {
    ...
    if (A === 0 || V === 0) {
      ...
      const humanoGanhou = (winner === "Azuis"); // jogador local é sempre Azuis
      playSound(humanoGanhou ? "win" : "lose");
