...
function getValidMoves(pieces, fromIndex, steps, size, playerColor) {
  ...
  // Primeira jogada: peça que nunca se moveu só pode mover com valor 1
  ...
  const fromCoord = indexToCoord(fromIndex, size);
  
  for (const destIndex of frontier) {
    const destCoord = indexToCoord(destIndex, size);
    
    // Não pode VOLTAR para a linha inicial se já saiu dela
    // (só bloqueia se a peça não está atualmente na linha inicial)
    if (destCoord.row === initialRow && piece.inMotion && fromCoord.row !== initialRow) {
      continue;
    }
